    ...
    jwt.verify(
      ...
      (err, decoded) => {
        ...
        for (let i = 0; i < toSends.length; i++) {
          ...
          redisMulti.lpush('chatUp:room:r_m_' + toSends[i].channel, JSON.stringify(toSends[i]))
          redisMulti.ltrim('chatUp:room:r_m_' + toSends[i].channel, 0, parent._conf.messageHistory.size - 1)
          redisMulti.expire('chatUp:room:r_m_' + toSends[i].channel, parent._conf.messageHistory.expire)
        }
